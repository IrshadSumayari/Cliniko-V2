    ...
    try {
      ...
      switch (pmsType.toLowerCase()) {
        case "cliniko":
          ...
          const clinikoResponse = await fetch(
            "https://api.cliniko.com/v1/patients",
            {
              headers: {
          ...
          testMessage = isValid
            ? "Cliniko connection successful"
            : "Cliniko connection failed";
          break;

        case "halaxy":
          // Test Halaxy API connection
          const halaxyResponse = await fetch(
            "https://api.halaxy.com/api/v1/patients",
            {
              headers: {
                Authorization: `Bearer ${apiKey}`,
                Accept: "application/json",
          ...
          testMessage = isValid
            ? "Halaxy connection successful"
            : "Halaxy connection failed";
          break;

        case "nookal":
          // Test Nookal API connection
          const nookalResponse = await fetch(
            "https://api.nookal.com/v1/patients",
            {
              headers: {
                Authorization: `Bearer ${apiKey}`,
                Accept: "application/json",
          ...
          testMessage = isValid
            ? "Nookal connection successful"
            : "Nookal connection failed";
          break;
