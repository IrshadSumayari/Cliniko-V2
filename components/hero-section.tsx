    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-accent">
      ...
      <div className="container mx-auto px-6 py-16">
        ...
        <div className="text-center max-w-4xl mx-auto fade-in">
          ...
          <Button variant="hero" size="xl" className="mb-4" onClick={onGetStarted}>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                      ...
                      { label: "Sessions Remaining", value: "1,234", icon: Clock },
                      { label: "Clients Needing Action", value: "8", icon: Bell },
                      { label: "Last Sync", value: "2 min ago", icon: ArrowRight }
                    ].map((stat, index) => (
                      <div key={index} className="luxury-card p-4 text-center">
                  ...
                  <div className="space-y-2">
                    {[
                      { name: "Sarah M***", program: "EPC", sessions: "3/5", status: "good" },
                      { name: "John D***", program: "Workers Comp", sessions: "8/12", status: "good" },
                      { name: "Emma W***", program: "Workers Comp", sessions: "1/10", status: "warning" },
                    ].map((patient, index) => (
                      <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-muted/30 border border-border/50">
                        <div className="flex items-center gap-4">
                          ...
                          <span className={`program-badge-${patient.program.toLowerCase().replace('workers comp', 'workcover')}`}>
                            {patient.program}
                          </span>
                        </div>
                        <div className="flex items-center gap-4">
                          <div className="text-sm">{patient.sessions}</div>
                          <div className={`w-2 h-2 rounded-full ${patient.status === 'warning' ? 'bg-warning' : 'bg-success'}`}></div>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="text-center mt-16">
          <Button variant="luxury" size="xl" onClick={onGetStarted}>
            Start Tracking Now
            <ArrowRight className="ml-2 h-5 w-5" />
          </Button>
        </div>
